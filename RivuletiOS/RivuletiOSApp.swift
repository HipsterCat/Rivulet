...
struct RivuletiOSApp: App {
    ...
    init() {
        ...
        // DEBUG never starts Sentry (see SentryConfig). In release, an empty
        // baked-in DSN means there is nothing to start. The 3s wait keeps the
        // SDK's swizzling and session-tracking cost off the launch path.
