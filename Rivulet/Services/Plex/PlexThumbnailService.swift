...
final class PlexThumbnailService {
    ...
    ///
    /// `times` is generated by the caller as evenly-spaced samples across
    /// the FULL media duration (see `PlayerProgressBarView.beginFilmstripLoad`),
    ...
    /// duration). Requesting a time past the BIF's last real frame doesn't
    /// error — `BIFData.frameIndex(at:)` clamps it to `frames.count - 1` —
    /// so every tail sample past that point silently collapses onto the
    /// SAME final frame. For a long tail (e.g. a chapterless episode whose
    /// BIF stops well short of the runtime), that reads as several
    /// identical tiles bunched at the end of the ribbon. Remapping
    /// out-of-coverage times to spread proportionally across the BIF's own
    /// covered range keeps every tile position showing a distinct real
    /// frame instead of repeating the last one.
    func filmstrip(partId: Int, times: [TimeInterval], maxPixelWidth: CGFloat,
        ...
        let indices = Self.frameIndices(forTimes: times, bif: bif)
    ...
    /// collapses onto the SAME final frame. For a long tail (e.g. a
    /// chapterless episode whose BIF stops well short of the runtime), that
    /// reads as several identical tiles bunched at the end of the ribbon.
    ///
