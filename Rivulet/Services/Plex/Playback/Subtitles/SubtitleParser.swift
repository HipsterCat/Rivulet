...
//  WebVTT parser.
//
//  The app no longer renders app-parsed subtitles (captions come from
//  AetherEngine, or from AVPlayer's legible output on Live TV's remote-HLS
//  path). This survives for ONE live caller: the content filter parses
//  `text/mcf+vtt` filter documents with it. See ContentFilterParsers.
//
