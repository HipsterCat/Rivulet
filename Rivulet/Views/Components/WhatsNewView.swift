...
struct WhatsNewView: View {
    ...
    static let changelogs: [(version: String, features: [String])] = [
        ("1.0.4 (70)", [ // confirm build number at release
            "The TV guide now refreshes itself when you come back to the app, and again whenever you open Live TV and the listings are more than half an hour old, so you no longer arrive at an empty or out of date guide",
            "The Live TV player controls now include a channel list, opening on the channel you are watching, with programme artwork and what is on right now for every channel",
            ...
            "Updated AetherEngine to 5.23.11. Fixes titles that never finished playing at the very end, resuming near the end of a movie getting stuck on Loading, Dolby Vision being dropped when you resume close to the end, and a crash after several minutes of playback on fast connections. Also: some MP4 movies that opened to a frozen frame now play and use less power, Dolby Vision titles streamed from a slow server no longer jump the scrubber backward or stall when you seek, the Atmos badge is now accurate, long Live TV sessions no longer slowly use up memory until the app closes, and leaving a Dolby Atmos title no longer leaves your receiver looping the last sound",
            "More engine fixes: web downloads with Dolby Digital Plus or Atmos audio that quietly fell back to a server transcode now play untouched, some titles with FLAC audio that failed to open now play, a title played by software decode no longer freezes its picture for good while the sound keeps going, and subtitles that stopped part way through a title and never came back now keep loading",
            "More playback fixes: HEVC videos in MP4 files that would not open on Apple TV now play, the last few minutes of a title now play through instead of stopping with an error, and resuming part way into a title starts faster on slower connections",
            "Live TV fixes: interlaced channels now play with sound, channels that showed a black screen with working audio now play, HDR channels switch the TV into HDR instead of playing black, green flashes no longer return later in a channel, channels that rebuffered over and over when you opened them now start cleanly, a channel that loses its source now recovers on its own instead of restarting every few seconds, and leaving a channel you have watched for a while no longer freezes the app",
